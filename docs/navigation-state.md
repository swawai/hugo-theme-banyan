...
在语言页明确选择语言后，`preferences/language.js` 用当前标签页的
`sessionStorage["banyan:language-return"]` 记录一次语言代码。首个非语言页恢复时
立即消费该记录：历史恢复（含 `pageshow.persisted` 的 BFCache 恢复）使用本页
Hugo 译文映射打开所选语言版本，普通导航只清除记录并尊重链接本身的语言。
译文跳转使用 `location.replace()`，保留查询参数与锚点，不增加历史记录；
目标页继续按既有协议校验来源和排序。没有译文时保留原内容并显示说明。
不同语言的段落及锚点可能不同，不保证正文滚动位置逐像素一致。

这条一次性记录不保存网址，也不使用全局 `preferred_lang` 强制改写页面：
