...
`canvas-position.js` 在新访问解析出 `#main` 时，根据实际视觉视口判断主列是否可见，再调用原生 `scrollIntoView`。浏览器自行处理桌面的文档滚动与手机的视觉视口平移；不通过限制缩放或另建滚动容器区分设备。内联入口位于头部样式之后；主列宽度与前置骨架须在主列出现前确定，`scroll-margin` 保留主列起点留白。该初始化只执行一次；已有滚动位置、锚点或加载期间的用户操作优先，历史返回、前进和刷新使用浏览器原生恢复，排序和异步重绘不重置画幅。

修改路径排序协议参见 [navigation-state.md](navigation-state.md)。黑白配色保留为下一独立步骤。
