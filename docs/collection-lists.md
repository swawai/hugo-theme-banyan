...
外观的三个子项在 `layouts/_default/appearance-page.html` 中通过 `iconText` 分别使用 `◐`（跟随系统）、`☀︎`（浅色）、`☾`（深色）；太阳包含 `U+FE0E`，请求文本样式。第一列入口继续使用页面声明的 `icon: theme` SVG。文字图标复用固定宽度图标槽，并从按钮的可访问名称中排除。
