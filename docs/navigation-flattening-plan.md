...
3. **系统页面（已完成）**：完成语言、外观及现有“我的”页接入，再完成站点页的版本与更新操作，同属第 3 步验收。复用已有偏好和更新逻辑，将操作逻辑与下拉菜单的 DOM、脚本加载条件分开；全站主题初始化、跟随系统、语言推荐及更新提示继续有效。语言切换留在对应语言的设置页；原阅读地址及状态仅供显式“返回”操作使用，独立于文章的 `from`。“我的”保持现有功能范围，不扩成账户系统。旧控件呈现在第 4 步删除，共存期间共用操作逻辑。
...
复测微调：用户要求切换语言后保留设置页位置。已将语言选项指向设置页自身的译文，保存语言偏好；只有显式“返回”才打开原阅读地址。删除读取原页面译文的请求、加载／失败／重试 UI。三语言切换、刷新、显式返回及无效／不可读取来源纳入回归。此规则替代历史第 3 步中“切换后打开原文章译文”的行为。生产构建 `temp_workspace/public/2609080954-language-stay`、129 个 HTML 审计及 4 项语言／设置关联回归通过；保留“我的”返回检查后补跑语言用例通过。
...
旧来源边界已确定并验证：文章书签的 `from=product-categories/free` 不再匹配当前来源，`from=products` 也不能继续表示产品全部；两者按原有无效来源规则恢复文章真实目录，第一列选中“目录”。新链接使用 `from=products/free` 和 `from=all-products`。路径重定向只处理旧分类页面网址，不隐式改写文章查询参数；系统 `return` 保留原返回地址，返回后应用同样的有效来源规则，不新增运行时旧名称适配。
