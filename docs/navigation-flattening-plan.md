...
继续完成弹窗移除之外的清单：取消全站 hover 透明度与列表数据格的 pointer-events 屏蔽，修正正文长链接断行和末尾引用间距，删除默认值重复声明。集合容器、网格、单元格和元信息统一按所属组件命名；SSR、客户端路径列和排序一并迁移到明确的 `data-collection-*` 行为标记。保留既有列宽、选中与焦点、大画幅及颜色。

语言选项由 Hugo 直接输出，页内操作由 `language-page.js` 处理，跨页返回由 `language-return.js` 读取标准译文链接；删除重复语言 JSON、旧 `return` 清理层和没有消费者的主题事件／镜像状态。更新引擎保留全站运行，只有检查页加载 `updates/page.js`。小型共享 CSS 仍合入 `page.css`，不机械拆成许多独立请求。具体位置与加载表见 [CSS 结构收尾](css-structure-review.md#选择器与页面资源收尾2026-09-10)。

## 前一增量：移除全站语言推荐与更新弹窗

用户确认先移除两种全站弹窗。语言不再根据浏览器语言推荐或跳转；同步删除推荐文案、延时监听及推荐专用 localStorage 读写。显式语言链接、设置页历史替换、返回时匹配原页面译文的 `banyan:language-return` 会话记录继续工作。没有消费者的 `preferred_lang` 不再写入；不增加旧存储键迁移代码。

PWA 在普通页面静默发现 waiting worker，更新目录仍为普通入口，检查页呈现状态并提供检查／立即更新按钮。删除原生更新确认、可见控件探测、弹窗一次性标记及三语言 prompt 字段。注册、后台检查、用户主动刷新时应用 waiting worker、激活失败恢复与缓存策略维持原有行为。

该轮仅处理弹窗；其后的全局／页面 JS 包与集合 DOM 类名已在上方增量继续完成，黑白配色仍独立验收。测试运行器不再为隐藏语言弹窗预写状态，新增不同浏览器语言的首次访问／刷新验证，更新原有 PWA 场景为静默发现与手动应用。

验收：两份生产构建 `temp_workspace/public/2609101122-no-prompts-from-2` 与 `temp_workspace/public/2609101124-no-prompts-to` 成功，后者 141 页 HTML 审计通过。16 项定向浏览器回归全部通过，报告为 `temp_workspace/regression/260910112414-browser/report.json`。覆盖中英文浏览器首次访问与刷新无推荐、语言返回／真实 BFCache／缺失译文／存储受限、外观同步、静默发现更新／隐藏按钮无弹窗、手动检查及应用、离线重试、旧导航缓存清理、首次导航及 CSS／JS 预缓存、离线重载、`sw.js` 不入缓存及其 `no-cache, max-age=0, must-revalidate` 响应头。

## 前一增量：CSS 与黑白主题

布局目录迁移已验证并合回开发分支。[CSS 审查与黑白主题实施计划](css-review.md) 将第 6 步细分为：6A 基础职责及交互状态收敛、6B 正文与样式装配减负、6C 黑白灰视觉验收。6A／6B 及后续结构补充已完成：配色、画幅、表格和正文组件职责归位，条目状态与焦点统一，多载／漏载修复；`assets/css/` 保留 19 个扁平职责文件，完整构建只发布 6 个 CSS。公共页面统一使用 `page.css`，有正文的页面再取得单一 `prose.css`。保留当前颜色，验收后再进入 6C。

6A／6B 第一轮的生产构建与 141 页产物检查通过；40 项既有浏览器回归、新增共享交互检查及三语言集合契约通过。结构补充把 `base`／`icons`、三个 `collection-*`、`document-meta` 与七个 `prose-*` 职责显式化，并由 `baseof.html` 直接装配公共样式。具体阶段产物、当前结构和视觉对比记录见 [CSS 实施结果](css-review.md)。
...
仍在工作的实现只做职责收敛：`baseof.html` 直接装配唯一公共 `page.css`，真正输出 `.prose` 的入口再装配唯一 `prose.css`；语言、外观、我的图标的 SVG 几何统一由 `data/icons.toml` 声明；页面 shell 只保留实际被 CSS 使用的状态类；JS 模块不再导出仅供自身调用的符号。活跃模板与内容同步迁移到 Hugo 当前的 `hugo.Sites`、`hugo.Data`、`cascade.target` API；构建版本改为最后写入的就绪标记，并新增构建时间完整性检查，消除并发渲染偶发生成空时间的竞态。动态 breadcrumb、来源恢复、横向画幅位置、排序、SW 停用路径、迁移跳转和内容作者 shortcodes 均保留。
...
1. **名称列表与样式拆分（已验收）**：增加 `list: name`，仅显示图标和名称，默认名称升序，完整复用集合来源、继承、排序、路径列和历史。当前由 `collection-item.css` 统一条目状态，`collection-grid.css` 提供单列和列头基底，`collection-table.css` 提供多列表格与 directory／all／products 列定义；DOM 继续使用 `.grid-list` 和 `.grid-*` 变体。choice 共用基底，保留原生链接／按钮及各自功能逻辑，不增加排序。不引入 `list: base`、`list_columns` 或 `grid-name.css`。
2. **更新入口迁移（已实施，待用户确认）**：主题 `content/updates/_index*.md` 使用 `layout: page-collection`、`list: name`、`root_nav: true`；`updates/check/index*.md` 使用 `page-update-check` 布局承载现有检查操作，`updates/changelog/index*.md` 承载更新记录，项目同路径内容同步迁移。检查页内显示实际版本／状态，版本为普通文本，不再嵌套更新记录链接；相关 `changelog_page` 字段和模板耦合已删除。名称列表不伪造运行时更新时间。旧 `pwa/`、`site/` 内容入口及 `pwa-page` 布局已删除，第一列合并为“更新”，共 14 项；模型、引用、sitemap、导航及对应测试已迁移。`assets/site/pwa/` 资源路径不变。更新记录保留 `/changelog/`，旧 `/pwa/`、`/site/` 通过根部署规则分别永久跳转到检查页和更新列表，覆盖三语言与有无尾斜杠。
...
第一列复用第二、三列的 `system-ui/list/link-cell.html`、`system-ui/list/item-content.html` 和 `collection-item.css`，沿用行结构、图标、间距、悬停及 `.is-current` 选中态；适配容器与断点限制。统一入口列表保持完整，来源变化只更新选中项，避免原根菜单渲染函数把它替换成局部菜单。
...
6. **配色收敛（6A／6B 与结构补充已实施，6C 待实施）**：样式职责、交互状态与正文加载已整理，19 个源码按职责维护并发布为 6 个稳定资源，保留清楚的选中态、键盘焦点和稳定的画幅位置；下一步依据 [CSS 审查](css-review.md) 完成黑白灰视觉验收。
...
入口显式声明：根页面只有顶层 `root_nav: true` 才显示，未声明默认隐藏。三语言可见根页面及首页已补齐声明，前四项采用“文章 - 全部、文章 - 分类、产品 - 全部、产品 - 分类”，权重依次 10、20、30、40。目录与阅读目的保留真实页面、集合数据和文章底部链接，第一列隐藏后共 15 项。隐藏来源及直接访问目录下文章时，第一列无选中项，右侧仍显示真实路径；不新增菜单归属映射。仅在 `feature-browse/navigation/root/render.html` 过滤展示，完整根模型不变，无新增 JS 或样式分支。下面 17 项入口等记录描述此前步骤。
...
站点 Logo：三语言站点入口声明 `icon: { image: "site/pwa/favicon.svg" }`，通用图片图标改为调用现有 `system-assets/publish-local.html`，复用页面 bundle／全站 assets 的既有解析和哈希发布规则。入口 Logo 与 HTML 的 favicon 共用 `/site/pwa/favicon.<sha256>.svg`；不复制 Logo、不增加发布器或 JS 分支。图片目录默认值和自身声明均支持 assets，既有 ICP bundle 图片继续使用原哈希路径。
...
图片图标与入口排序：备案入口改为 `weight: 105`，排在版权入口 `110` 前面；三语言声明 `icon: { image: "0.webp" }`，资源来自项目 `content/icp/0.webp`。通用图标增加图片值，声明时以所属页面解析，再复用现有 `system-assets/publish-page.html` 发布到 `/media/content/icp/0.<sha256>.webp`；主表、路径列、图标默认值继承与 JSON 均保留同一地址。图片使用共用占位及 `1rem` 图形尺寸，保持比例；备案页保留 `build.publishResources: false`，只发布哈希资源。不改变缓存策略或新增图片发布链。
...
内容图标声明与继承：新增 `icon`、`list_icon_folder`、`list_icon_file` 的统一处理。`system-ui/list/icon-defaults.html` 为列表分别查找最近祖先的两种默认值；`system-ui/list/item-icon.html` 优先选择条目自身的非空 `icon`，否则使用当前列表默认值，最终为 `folder`／`file`。不按同名字段继承 `icon`，不沿 `aggregate` 继承；普通文章误写子项默认字段或使用未定义的图标时构建报错。共享列表行、第一列、站点列表及首页快捷列表采用相同规则，路径菜单转换保留 `icon`。产品分类根与产品全部的三语言内容各声明 `list_icon_file: product`，分类子页自动继承，Xvenv 无需逐篇声明。
...
5A 实施结果：现归入 `assets/js/browse/canvas-position.js` 的画幅逻辑只传递同标签页的一次列表导航横向位置；记录来源、目标与视觉视口坐标，目标页读取后立即清除，只有来源／目标相符的新访问才使用。普通直接访问不再自动显示主列，刷新与历史恢复继续由浏览器处理。沿用现有 main 列作为原生滚动目标，通过临时滚动留白还原坐标，兼顾桌面布局视口和手机视觉视口，不增加滚动容器或设备模式。用户输入／锚点优先。当时位于 `grid-products.css`、现归入 `collection-table.css` 的产品名称列使用共享 `--navigation-column-inline`（15rem）。
...
具体实现：`feature-browse/collection/config.html` 解析本页或最近祖先的 `list`；`feature-browse/collection/rows.html` 使用目录／分类原生成员或入口的 `aggregate`，主表、路径菜单、来源 JSON 共用结果。`feature-browse/collection/render.html` 按声明绘制三种表格，`feature-browse/collection/products/offer.html` 只读可选展示信息。旧产品筛选器、provider 与路径猜样式、旧 shortcode 及重复渲染器已删除。Xvenv 的三语言标注与四个可选分类内容文件在根项目；主题只保留分类根和汇总入口。
...
后续验收重点：第 4 步验证文章从目录／标签／阅读目的／全部／产品分类进入时，第一列始终完整、选中正确，刷新和前进后退不回写旧菜单；同时从新入口重跑第三步的系统页回归。若修改现归入 `assets/js/pwa/update-engine.js` 的更新引擎，仍执行 AGENTS.md 第 9 条规定的缓存及更新验证。第 5 步在 390、1024、1440 宽度验证同一列结构、阅读与返回位置、触摸及键盘可达。第 6 步的黑白灰配色与浅色／深色／跟随系统偏好分别处理。
...
第 4 步结构：`feature-browse/navigation/root/pages.html` 合并首页直接子页与真实分类法根，按页面 weight 排序；第一列只输出一份共享列表，站点名列头返回首页。内容祖先决定直接访问的归属，有效 `from` 决定来源归属；完整入口表不再复制进每份集合数据。同步内联初始化、首帧预览和运行时共用选中函数，慢速外部 JS 加载也不会先高亮默认目录。第一列与路径列使用同一行组件、225px 列宽及选中／悬停样式。页脚随导航自然排列，窄屏仍保持当前上下布局。
