...
2026-09-07｜分支：`codex/flatten-navigation`｜状态：第 4 步完成，内部 review 后补充修正与验收边界；第 5、6 步尚未实施
...
5. **布局合并**：内部按“普通列表列结构 → 画幅与初始位置 → 历史／锚点／键盘验收”推进，作为完整一步验收。统一各宽度的列结构，并删除路径下拉菜单的模板、控制器、加载开关及样式；同步处理静态模板、浏览器重绘和首屏列数预留。沿用文档滚动容器承担整页横向画幅，优先保留浏览器原生历史恢复；新访问的文章、集合和系统页应使当前主内容列可见，已有恢复位置或锚点时不再强制定位。正文按视口可用宽度连续换行并保留阅读宽度上限；表格、代码等按内容需要局部溢出。统一 DOM、视觉与键盘顺序，移除当前窄屏页脚的 CSS 重排。验证排序、刷新、前进后退、锚点、触摸及键盘行为，保留现有来源协议。
