...
2026-09-08｜分支：`codex/flatten-navigation`｜状态：5A 已实施并通过检查，等待用户复测；已确定 `products`＋`offer`，5B／5C 尚未实施；第 6 步尚未开始
...
2. **产品整理（初版已完成，规则待替换）**：已建立产品全部及分类入口，产品内容保持一份。当前仍按价格和来源筛选四个固定分类；此规则由下面的 5C 替换为作者自由声明的 Hugo 分类。
...
已确定的命名（2026-09-08 用户确认 `products`＋`offer`；运行中的代码与内容尚未迁移）：
...
根项目的 `content/d/products/xvenv/index.zh.md` 将按以下结构迁移（只展示相关字段，当前尚未改动）：
...
具体改动位置：主题 `layouts/partials/products/source-scope.html` 的固定目录筛选应删除，`collection/rows-products.html` 改用明确的成员集合，`products/model.html` 改读可选的 `offer.amount`／`offer.currency`／`offer.value`，并删除来源限定与价格分类推导；`entry-source/` 的注册与主列表接入 5B 的统一结果。分类根与自动 term 的布局由主题支持，Xvenv 的三语言业务标注在根项目调整。主题原 `content/products/` 全部入口迁到 `content/all-products/`，原 `content/product-categories/` 分类根迁到 `content/products/`；现有四个分类的业务名称／排序如需保留，在根项目内容中提供，不再作为主题固有分类。

迁移同时更新根项目 `data/redirects.toml`，旧 `/product-categories/.../` 如需保留，应直接转往新 `/products/.../`，避免重定向链。现有规则中 `/products/first-party/`、`/products/third-party/` 及各语言／无尾斜杠变体会与新的真实分类页面重叠，必须按新页面地址删除或重新确定规则，不能机械替换目标造成自跳转。原 `/products/` 改为分类根后不能再重定向到全部入口；原来指向产品全部的站内链接、关于页与测试要明确改为 `/all-products/`。不保留两份内容树或两套分类逻辑，同步核对 canonical、三语言入口、首帧及运行时选中状态。计划与命名研究不等于实现完成；5B 的 `list` 字段和 5C 的原生产品分类目前均未接入。

改名验收还需单独覆盖文章书签里的 `from=product-categories/free`（新语义为 `products/free`）和 `from=products`（旧语义为产品全部，新位置是 `all-products`）：路径重定向不会自动改写文章查询参数。明确旧来源的迁移或失效／语义变化后的行为，不能把分类页跳转成功当作来源语义也已保留；系统链接携带的 `return` 同样纳入核对，不在新集合模型里长期支持两套分类名称。

本轮计划依据：根项目配置与产品 front matter、主题集合／分类／排序实现，以及前轮针对根内容的 Hugo 分类探针；原生配置约定参考 [Hugo 分类配置](https://gohugo.io/configuration/taxonomies/)。本轮仅修订计划，不以旧回归通过记录替代上述新要求的验收。
