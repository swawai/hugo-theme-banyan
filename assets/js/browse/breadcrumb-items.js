...
export const normalizeBreadcrumbCollectionSource = normalizeCollectionSource;
...
export function buildPathColumnItems(decoded, collectionSource, selection = {}) {
    if (!collectionSource?.logicalPath || !supportsItemsPayloadProvider(collectionSource.provider)) {
        return [];
    }
...
export function buildSelectedBreadcrumbItem(source, selectedPathname = '', selectedTitle = '') {
    ...
    if (!source || !normalizedSelectedPathname) {
        return null;
    }
    ...
    const decoded = source.currentCollectionItems;
    if (!decoded) {
        return null;
    }
